...
1. **测试配置**: 项目默认跳过测试 (`<skipTests>true</skipTests>`)
2. **前端集成**: 前端构建产物需放在 `src/main/resources/static/` 目录
3. **认证机制**: 使用 Sa-Token 进行权限管理，GitHub OAuth 作为用户认证
4. **日志系统**: 使用 SLF4J + MyBatis 日志实现
5. **全局异常处理**: `GlobalRestExceptionHandler` 统一处理异常
6. **跨域配置**: `CorsConfig` 处理前后端分离的跨域问题
